...
export class SignupFormComponent {
  ...
  samePassword: ValidatorFn = (form) => {
    ...
    const confirmPasswordControl = (form as FormGroup).controls[
      'confirmPassword'
    ];
  ...
  passwordNoUsernames: ValidatorFn = (form: AbstractControl) => {
    ...
    const isPasswordIncludes = (search: string) =>
      formValue.password?.toLowerCase().includes(search);

    if (
      isPasswordIncludes(formValue.firstName?.toLowerCase()) ||
      isPasswordIncludes(formValue.lastName?.toLowerCase())
    ) {
